...
module.exports =
class MinimapGoogleDiffBinding
  ...
  constructor: (@minimap) ->
    ...
    unless @minimap?
      return console.warn 'minimap-google-diff binding created without a minimap'
  ...
  addDecorations: (diffs) ->
    for {oldStart, newStart, oldLines, newLines} in diffs
      ...
      if oldLines is 0 and newLines > 0
        @markRange(startRow, endRow, '.minimap .google-diff-line-added')
      else if newLines is 0 and oldLines > 0
        @markRange(startRow, startRow, '.minimap .google-diff-line-removed')
      else
        @markRange(startRow, endRow, '.minimap .google-diff-line-modified')
